...
EllipsoidalMesh::EllipsoidalMesh(const Eigen::ArrayXd& x, const Eigen::ArrayXd& y, const Eigen::ArrayXd& z, double a, double b, double c):
    SpatialMesh(x, y, z),
    _a(std::max({a, b, c})),
...
EllipsoidalMesh::EllipsoidalMesh(const std::array<Eigen::ArrayXd, 3>& axes, double a, double b, double c):
    SpatialMesh(axes),
    _a(std::max({a, b, c})),
    _b(a+b+c - std::max({a, b, c}) - std::min({a, b, c})),
    _c(std::min({a, b, c})),
...
EllipsoidalMesh::EllipsoidalMesh(const Eigen::ArrayXd& x, const Eigen::ArrayXd& y, const Eigen::ArrayXd& z, const Ellipsoid& ell):
    EllipsoidalMesh(x, y, z, ell.semiAxis(0), ell.semiAxis(1), ell.semiAxis(2))
{}

EllipsoidalMesh::EllipsoidalMesh(const std::array<Eigen::ArrayXd, 3>& axes, const Ellipsoid& ell):
    EllipsoidalMesh(axes, ell.semiAxis(0), ell.semiAxis(1), ell.semiAxis(2))
{}

Eigen::Vector3d EllipsoidalMesh::cartesian(double r, double mu, double phi) const noexcept{
    return {_a*r*mu, _b*r*std::sqrt(1-mu*mu)*std::cos(phi), _c*r*std::sqrt(1-mu*mu)*std::sin(phi)};
}

Eigen::Vector3d EllipsoidalMesh::coordinates(double x, double y, double z) const noexcept{
    x /= _a;
    y /= _b;
    z /= _c;
...
Eigen::Vector3d EllipsoidalMesh::basisVector(Eigen::Index varID, double r, double mu, double phi) const noexcept{
...
UnitVector3d EllipsoidalMesh::normalVector(Eigen::Index varID, double r, double mu, double phi) const noexcept{
    if (varID == 0){
...
double EllipsoidalMesh::gradientDotN(double dudq, Eigen::Index i, Eigen::Index j, Eigen::Index k, Eigen::Index surfID) const{
...
double EllipsoidalMesh::area(Eigen::Index i, Eigen::Index j, Eigen::Index k, Eigen::Index surfID) const noexcept{
    double del = _c/_b;
    double eps = _c/_a;
    double S;
    if (surfID <= 1){
...
double EllipsoidalMesh::volume(Eigen::Index i, Eigen::Index j, Eigen::Index k) const noexcept{
...
double EllipsoidalMesh::ghostLength(Eigen::Index varID) const noexcept{
