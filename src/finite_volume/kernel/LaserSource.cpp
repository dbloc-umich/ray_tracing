...
LaserSource::LaserSource(std::shared_ptr<Material> mat, const Eigen::VectorXi& s, Ray& ray, Shape* shape):
    Kernel(mat, PropVariable::temperature, s),
    _ray(ray),
    _shape(shape),
    _I0(ray.intensity())
{
...
Eigen::MatrixXd LaserSource::computeResidual(const StateMesh& u) const{
    auto mesh = u.mesh();
    ...
    Eigen::MatrixXd q = Eigen::MatrixXd::Zero(Ns, Nr*Nmu*Nphi);
    ...
    */
    ...
    double n1 = 1.0;
    double T = _mat->T_from_H(u(_s[0],i,j,k));
    Material::PropVars vars{{_var, T}};
    ...
    while (true){
        ...
        // Update heat and ionization sources
        ...
        q(0, (i*Nmu + j)*Nphi + k) = Id*_shape->surfaceArea(); // energy term, assuming laser cross sectional area > droplet surface area

        double Na = _mat->computeProperty(Prop::numberDensity, vars);
        double sigma = _mat->computeProperty(Prop::photoionizationCrossSection, vars) * 1e-28; // 1e-28 = conversion factor from barn to m2
        double hnu = pconst::h * ray.frequency();
        double V = mesh->volume(i,j,k);
        q(1, (i*Nmu + j)*Nphi + k) = Na*sigma*Id/hnu*V; // photoionization term
