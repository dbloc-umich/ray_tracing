...
Eigen::MatrixXd LaserSourceKernel::computeResidual(const StateMesh& u) const{
    auto mesh = u.mesh();
    ...
    */
    ...
    double hnu = pconst::h * ray.frequency() / (2*mconst::pi);
    ...
    while (true){
        ...
        if (hnu >= Eb){
            ...
            if (N > 4) throw std::runtime_error("ERROR: N-photon ionization rate coefficient for N > 4 has not been implemented.");
            double SN;
            if (N == 2) SN = _mat->computeProperty("2-photon_ionization_rate_coefficient", vars);
            else if (N == 3) SN = _mat->computeProperty("3-photon_ionization_rate_coefficient", vars);
            else SN = _mat->computeProperty("4-photon_ionization_rate_coefficient", vars);
