...
FVLaserSource::FVLaserSource(std::shared_ptr<Material> mat, Eigen::Index s, Ray& ray, Shape* shape):
    FVKernel(mat, PropVariable::temperature, s),
    _ray(ray),
    _shape(shape),
    _I0(ray.intensity())
{
...
Eigen::VectorXd FVLaserSource::computeResidual(const FVStateMesh& u) const{
    ...
    Eigen::VectorXd q = Eigen::VectorXd::Zero(Nr*Nmu*Nphi);
    ...
    */
    ...
    double n1 = 1.0;
    double T = _mat->T_from_H(u(_s,i,j,k));
