...
FVDiffusion::FVDiffusion(std::shared_ptr<Material> mat, PropVariable var, Eigen::Index s, Prop prop):
    FVKernel(mat, var, s),
    _prop(prop)
{}
...
Eigen::VectorXd FVDiffusion::computeResidual(const FVStateMesh& u) const{
    ...
    Eigen::VectorXd F = Eigen::VectorXd::Zero(Nx*Ny*Nz);
            ...
            for (Eigen::Index j = 0; j < Ny; j++){
                for (Eigen::Index k = 0; k < Nz; k++){
                    ...
                    for (auto surfID: surfaces){
                        ...
                        F[(i*Ny + j)*Nz + k] += flux;
                        F[(in*Ny + jn)*Nz + kn] -= flux;
                    }

                    // Flux from boundaries
                    if (i == 0){
                        Eigen::Vector3d r{ mesh->axis(0)[i], (mesh->axis(1)[j]+mesh->axis(1)[j+1])/2, (mesh->axis(2)[k]+mesh->axis(2)[k+1])/2};
                        F[(i*Ny + j)*Nz + k] += u.bc(0)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,0);
                    }
                    if (i == Nx-1){
                        Eigen::Vector3d r{ mesh->axis(0)[i], (mesh->axis(1)[j]+mesh->axis(1)[j+1])/2, (mesh->axis(2)[k]+mesh->axis(2)[k+1])/2};
                        F[(i*Ny + j)*Nz + k] += u.bc(1)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,1);
                    }
                    if (j == 0){
                        Eigen::Vector3d r{ (mesh->axis(0)[i]+mesh->axis(0)[i+1])/2, mesh->axis(1)[j], (mesh->axis(2)[k]+mesh->axis(2)[k+1])/2};
                        F[(i*Ny + j)*Nz + k] += u.bc(2)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,2);
                    }
                    if (j == Ny-1){
                        Eigen::Vector3d r{ (mesh->axis(0)[i]+mesh->axis(0)[i+1])/2, mesh->axis(1)[j], (mesh->axis(2)[k]+mesh->axis(2)[k+1])/2};
                        F[(i*Ny + j)*Nz + k] += u.bc(3)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,3);
                    }
                    if (k == 0){
                        Eigen::Vector3d r{ (mesh->axis(0)[i]+mesh->axis(0)[i+1])/2, (mesh->axis(1)[j]+mesh->axis(1)[j+1])/2, mesh->axis(2)[k]};
                        F[(i*Ny + j)*Nz + k] += u.bc(4)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,4);
                    }
                    if (k == Nz-1){
                        Eigen::Vector3d r{ (mesh->axis(0)[i]+mesh->axis(0)[i+1])/2, (mesh->axis(1)[j]+mesh->axis(1)[j+1])/2, mesh->axis(2)[k]};
                        F[(i*Ny + j)*Nz + k] += u.bc(5)->computeFlux(u.cell(i,j,k), r)[_s] * mesh->area(i,j,k,5);
                    }
                }
            }
        }
