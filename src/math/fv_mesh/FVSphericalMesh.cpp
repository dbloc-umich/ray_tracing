...
double FVSphericalMesh::gradientFactor(Eigen::Index varID, double r, double mu, double phi) const noexcept{
    if (varID == 0) return 1;
    if (varID == 1) return std::sqrt(1-mu*mu)/r;
    return 1.0/std::sqrt((1-mu*mu)*r);
}

double FVSphericalMesh::gradientDotN(double dudq, Eigen::Index i, Eigen::Index j, Eigen::Index k, Eigen::Index surfID) const{
...
double FVSphericalMesh::ghostLength(Eigen::Index varID) const noexcept{
