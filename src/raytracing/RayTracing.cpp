...
namespace{
    ...
    template<typename T>
    Shape* attenuate(const Tree<T>& tree, Ray& ray, double temp, double& s, int& count){
        ...
        Shape* next = tree.nextShape(ray, s, count);
#ifdef MONITOR
        ...
        std::cout << " Number of distance calculations: " << count << std::endl;
#endif
        if (next){
            ...
            if (ray.host() == next){
                ...
                double alpha = 0.0;
                if (next->hasProperty("attenuation_coefficient")){
                    alpha = next->computeProperty("attenuation_coefficient", varmap);
                } else if (next->hasProperty("extinction_coefficient")){
                    alpha = next->computeProperty("extinction_coefficient", varmap);
    ...
    std::vector<Ray> penetrate(const Tree<T>& tree, Ray& ray, double temp, Shape* next, double s, const HopMode& mode){
        ...
        **/
        ...
        if (!ray.host()){ // current points to empty space
            n1 = 1.0;
            n2 = next->computeProperty("refractive_index", varmap);
        } else if (s == 0.0){
            // particle enters directly from current to next
            n1 = ray.host()->computeProperty("refractive_index", varmap);
            n2 = next->computeProperty("refractive_index", varmap);
            ...
            n1 = ray.host()->computeProperty("refractive_index", varmap);
            int pseudo = 0;
            auto pseudoNext = tree.nextShape(ray, s, pseudo);
            n2 = (s == 0) ? pseudoNext->computeProperty("refractive_index", varmap) : 1.0;
        }
...
template<typename T>
double intensity(const Tree<T>& tree, Ray& ray, double temp, int& count, const HopMode& mode) noexcept{
    ...
    if (mode != HopMode::Detailed){
        double s;
        Shape* next = attenuate(tree, ray, temp, s, count);
        while(next){
            ...
            ray.setHost(next);
            next = attenuate(tree, ray, temp, s, count);
        }
...
#include "Node.h"
template double intensity(const Tree<Node>&, Ray&, double, int&, const HopMode&);

#include "UNode.h"
template double intensity(const Tree<UNode>&, Ray&, double, int&, const HopMode&);