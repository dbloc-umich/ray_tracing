...
When expressing this transfer under a summation operator, e.g., $\displaystyle \sum_{r} \Sigma_r^j \mathcal{E}_r$, $\Sigma_r^j$ indicates all reactions $r$ that produce particle $j$, and $\mathcal{E}_r$ means the
energy of the reactant in each reaction. Note that this only applies readily for heavy particles; electrons typically involve reaction-specific transfer terms that are described in Section \ref{sect:reactions}.
