...
class Simulation{
    ...
    void addKernel(std::shared_ptr<Kernel> kernel){ if (kernel) _kernels.push_back(kernel); }
    void setTimeIntegrator(std::shared_ptr<TimeIntegrator> integrator){ _integrator = integrator; }

    void solve(StateMesh& u, double ti, double tf, double dt) const;
