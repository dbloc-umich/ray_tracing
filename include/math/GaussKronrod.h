...
class GaussKronrodBase : public GaussLegendreBase{
    ...
    **/
    ...
    inline static const std::vector<std::vector<double>> _Lweights =
    ...
    inline static const std::vector<std::vector<double>> _Knodes =
    ...
    inline static const std::vector<std::vector<double>> _Kweights =
...
class GaussKronrod : public GaussianQuadrature<N, M>, GaussKronrodBase{
    ...
    Eigen::VectorXd getNodes(double l, double u) const override{
        Eigen::ArrayXd v(n());
        for (std::size_t i = 0; i < this->_n; i++) v[i] = _nodes[this->_n-1][i];
        for (std::size_t i = this->_n; i < n(); i++) v[i] = _Knodes[this->_n-1][i-this->_n];
        return v * (u-l)/2 + (u+l)/2;
    }

    Eigen::VectorXd getWeights(double l, double u) const override{
        Eigen::ArrayXd v(n());
        for (std::size_t i = 0; i < this->_n; i++) v[i] = _Lweights[this->_n-1][i];
        for (std::size_t i = this->_n; i < n(); i++) v[i] = _Kweights[this->_n-1][i-this->_n];
        return v * (u-l)/2;
    }
