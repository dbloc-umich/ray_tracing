...
template<int M>
CrankNicolson<M>::CrankNicolson(double t0, double t1, double dt, SolverPointer solver):
    ODE_IVPSolver<M>(t0, t1, dt),
    _nlSolver(std::move(solver))
{}

template<int M>
CrankNicolson<M>::CrankNicolson(const Eigen::ArrayXd& t, SolverPointer solver):
    ODE_IVPSolver<M>(t),
    _nlSolver(std::move(solver))
{}

template<int M>
ODEStatus CrankNicolson<M>::solve(const Function& f, const RangeType& ic) const noexcept{
